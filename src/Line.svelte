<script>
	...
	const changeOrder = () => {
		...
		if (typeof current !== "undefined") {
			order.salad =
				typeof current.salad !== "undefined"
					? salads[current.salad].name
					: "";
			order.size =
				typeof current.size !== "undefined" ? sizes[current.size] : "";
			order.bread =
				typeof current.pain !== "undefined" ? current.pain : 0;
		}
		sizeOrder();
	};
	const toggleBread = () => {
	...
	{#if order.salad !== salads[10].name}
		...
		<span on:click={toggleBread} style="top: 5px;position: relative;">
			<Icon
				data={faBreadSlice}
				scale="1.5"
				style="cursor:pointer; color:{order.bread
					? '#805828'
					: '#8058285e'};"
			/>
		</span>
